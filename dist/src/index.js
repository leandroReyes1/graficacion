...
const camRotXSlider = document.getElementById('camRotX');
const camRotYSlider = document.getElementById('camRotY');
...
function updateAccordionUI() {
    const btnGlobalAction = document.getElementById('btnGlobalAction');
    if (!btnGlobalAction)
        return;
    const btnOscilacionAction = document.getElementById('btnOscilacionAction');
    const hasCabeza = miCanvas.groupConfigs.some(c => c.name.toLowerCase().includes('cabeza') ||
        c.name.toLowerCase().includes('head') ||
        c.name.toLowerCase().includes('oscila') ||
        c.name.toLowerCase().includes('motor'));
    const hasAspas = miCanvas.groupConfigs.some(c => c.name.toLowerCase().includes('aspa') ||
    ...
    if (hasCabeza && hasAspas) {
        ...
        const aspaConfig = miCanvas.groupConfigs.find(c => c.name.toLowerCase().includes('aspa') ||
            c.name.toLowerCase().includes('fan') ||
            c.name.toLowerCase().includes('rotor'));
        const cabezaConfig = miCanvas.groupConfigs.find(c => c.name.toLowerCase().includes('cabeza') ||
            c.name.toLowerCase().includes('head') ||
            c.name.toLowerCase().includes('oscila') ||
            c.name.toLowerCase().includes('motor'));
        if (aspaConfig) {
        ...
        const swingSlideConfigs = miCanvas.groupConfigs.filter(c => (c.type === 'swing' || c.type === 'slide') &&
...
if (camRotXSlider) {
    camRotXSlider.addEventListener('input', () => {
        ...
        if (lbl)
            lbl.textContent = camRotXSlider.value;
        miCanvas.paint();
    });
}
if (camRotYSlider) {
    camRotYSlider.addEventListener('input', () => {
        ...
        if (lbl)
            lbl.textContent = camRotYSlider.value;
        miCanvas.paint();
    });
}
if (zoomSlider) {
...
window.addEventListener('mousemove', (e) => {
    ...
    if (miCanvas.camRotX > pitchLimit)
        miCanvas.camRotX = pitchLimit;
    if (miCanvas.camRotX < -pitchLimit)
        miCanvas.camRotX = -pitchLimit;
    // Sincronizar Sliders y Etiquetas de la interfaz
    if (camRotXSlider)
        camRotXSlider.value = Math.round(miCanvas.camRotX * 180 / Math.PI).toString();
    if (camRotYSlider)
        camRotYSlider.value = Math.round(miCanvas.camRotY * 180 / Math.PI).toString();
    const lblX = document.getElementById('lblCamRotX');
    if (lblX)
        lblX.textContent = Math.round(miCanvas.camRotX * 180 / Math.PI).toString();
    const lblY = document.getElementById('lblCamRotY');
    if (lblY)
        lblY.textContent = Math.round(miCanvas.camRotY * 180 / Math.PI).toString();
    prevMousePos = { x: e.clientX, y: e.clientY };
    miCanvas.paint();
});
