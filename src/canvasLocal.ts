...
export type AnimType = 'none' | 'spin' | 'swing' | 'slide';
...
export interface GroupConfig {
  ...
  axis: 'X' | 'Y' | 'Z';
  speed: number;
  maxVal: number; // Ángulo máximo en grados (swing) o distancia máxima en unidades (slide)
  currentVal: number; // Valor de animación actual (ángulo o distancia)
  targetState: 'open' | 'closed'; // Estado del toggle
  parentName?: string; // Nombre del grupo padre para transformaciones jerárquicas
  pivotX: number;
  pivotY: number;
  pivotZ: number;
}
...
export class CanvasLocal {
  ...
  public groupConfigs: GroupConfig[] = [];
  ...
  public loadModelFromText(text: string, filename: string = ''): { groups: string[], verticesCount: number, facesCount: number } {
    this.vertices = [];
    this.faces = [];
    this.edges = [];
    this.groups = [];
    ...
    if (uniqueGroups.length <= 1 && this.vertices.length > 0 && this.faces.length > 0 && isAnimatableModel) {
      ...
      this.vertices.forEach(v => adj.set(v.id, new Set<number>()));
      ...
      if (components.length >= 2) {
        ...
        components.forEach(comp => {
          ...
          comp.forEach(vid => {
            ...
            if (v) {
              if (v.x < compMinX) compMinX = v.x;
              if (v.x > compMaxX) compMaxX = v.x;
              if (v.y < compMinY) compMinY = v.y;
              if (v.y > compMaxY) compMaxY = v.y;
              if (v.z < compMinZ) compMinZ = v.z;
              if (v.z > compMaxZ) compMaxZ = v.z;
            }
          });
    ...
    this.groups.forEach(group => {
      ...
      if (lowerGroup.includes('_x')) axis = 'X';
      else if (lowerGroup.includes('_y')) axis = 'Y';
      else if (lowerGroup.includes('_z')) axis = 'Z';
  ...
  public getTransformedPoint(x: number, y: number, z: number, groupName: string): { x: number, y: number, z: number } {
  ...
  public getGroupConfig(name: string): GroupConfig | undefined {
    return this.groupConfigs.find(c => c.name === name);
  }

  // --- CONMUTAR ESTADO DE ANIMACIÓN DE UN GRUPO ---
  public toggleGroupState(groupName: string) {
  ...
  public isPointInPolygon(px: number, py: number, polygon: { x: number, y: number }[]): boolean {
    let inside = false;
  ...
  public distanceToSegment(px: number, py: number, x1: number, y1: number, x2: number, y2: number): number {
    const dx = x2 - x1;
    const dy = y2 - y1;
  ...
  public pickGroup(mouseX: number, mouseY: number): string | null {
    if (this.vertices.length === 0) return null;

    // 1. Proyectar y calcular la profundidad promedio de cada cara
    interface FaceDepth {
      face: Face;
      avgDepth: number;
    ...
    const facesWithDepth: FaceDepth[] = [];

    this.faces.forEach(f => {
      let sumDepth = 0;
      let validCount = 0;
      const poly2d: { x: number, y: number }[] = [];
    ...
    let closestEdgeGroup: string | null = null;
    ...
    if (this.faces.length === 0 && this.edges.length === 0) {
      let closestVertexGroup: string | null = null;
